...
def get_csv():
    wb = load_workbook(r'C:\Users\work\Downloads\이준재\A2221_반려동물_카페_20200428.xlsx', data_only=True)
    ws = wb['Sheet']
...
def save(arr):
    ...
    ws.append(('ID', '데이터 분류명', '데이터 구분', '주요내용 및 서비스', '기준년월일', '경과기간(개월)', '출처',
               ...
               # u        v             w               x       y       z
               '주소', '전화번호', '내 주변 애견카페', '운영시간', '평점', '입장료 유무'))
    # DB 모든 데이터 엑셀로
    cnt = 0
    ...
    for row in arr:
        ...
        s = row['t']  # 수정일
        t = row['u']  # 키워드 -- 공통 --
        u = row['c'] + ' ' + row['d']  # 도로명 주소 ||
        v = row['e']  # 전화번호 phone[1].strip()
        w = '' # row['v']  # 평점
        x = '' # row['x']  # 지번 주소
        y = '' # row['w']  # 운영시간
        z = '' # row['y']  # 응급실 유무
    ...
    wb.save('C:/workSpace/flask_crawling/originalDatas/A2221_반려동물_카페_20200717.xlsx')
# 내주변 애견카페
# 24시간 영업 애견카페
# 애견카페 평가/평점 랭킹
# 애견카페 입장료 유무
