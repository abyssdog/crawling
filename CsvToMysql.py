...
def get_csv():
    wb = load_workbook(r'C:\Users\work\Downloads\메타데이터 ver 0.2\A2224_동물체험농장 및 테마파크_20200721.xlsx', data_only=True)
    ws = wb['Sheet']
    ...
    for row in iter_rows:
        __dict = {
            ...
            'c': row[2].value,  # 데이터 구분
            'd': row[3].value,  # 주요내용 및 서비스
            'e': row[4].value,  # 기준년월일
            'f': row[5].value,  # 경과기간(개월)
            'g': row[6].value,  # 출처
            'h': row[7].value,  # 데이터 생성자
            'i': row[8].value,  # 데이터 저작권
            'j': row[9].value,  # 데이터 수집방법
            'k': row[10].value,  # 데이터 프로토콜
            'l': row[11].value,  # 데이터 소스
            'm': row[12].value,  # 데이터 저장소
            'n': row[13].value,  # 데이터 확장 방법
            'o': row[14].value,  # 데이터 확장 참조
            'p': row[15].value,  # 등록자
            'q': row[16].value,  # 수정자
            'r': row[17].value,  # 등록일
            's': row[18].value,  # 수정일
            't': row[19].value,  # 키워드  -- 공통 --
            'u': row[20].value,  # 주소
            'v': row[21].value,  # 전화번호
            'w': row[22].value,  # 운영시간
            'x': row[23].value   # 평점
