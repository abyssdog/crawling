...
class CrawlClass(object):
    ...
    def test_insert1(self):
        ...
        sql = """
            SELECT *
              FROM RAW_SCHEDULE
             WHERE CONVENTION_NAME = 'songdoconvensia'
        """
        curs.execute(sql)
        rows = curs.fetchall()
        cnt = 0
        for row in rows:
            ...
            if match2:
                ...
                executed = curs.execute(query,
                                        ('songdoconvensia', match[0],
                                         'songdoconvensia 인천광역시 연수구 센트럴로 123 {place}'.format(place=str_place),
                                         '인천광역시', '연수구', '센트럴로 123',
                                         d_start, d_end, '', '',
                                         str_phone,
                                         str_home, str_manage, str_host, str_money, '',
                                         pattern_url, 'songdoconvensia', reg_date))
        conn.commit()
        conn.close()
