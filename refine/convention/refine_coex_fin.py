...
class CrawlClass(object):
    ...
    def refine(self):
        ...
        for row in rows:
            ...
            pattern_supvsn = r'주관\<\/strong\>(.*)\<\/li\>'
            pattern_addt_dtl = r'\<strong\>개최장소 \: \<\/strong\>(.*)\t'
            pattern_date = r'\<strong\>개최기간 \: \<\/strong\>(.*)\<\/li\>'
            pattern_time = r'\<strong\>관람시간 \: \<\/strong\>(.*)\<\/li\>'
            pattern_cost = r'입장료\<\/strong\>(.*)\<\/li\>'
            ...
            if match:
                ...
                date_end = datetime.datetime.strptime(tempdate[date_index+1:len(tempdate)].replace('.', '-').strip(), '%Y-%m-%d')
