...
class CrawlClass(object):
    ...
    def refine(self):
        ...
        for row in rows:
            ...
            pattern_host = r'주최\<\/strong\>\n\<p\>(.*)\<\/p\>'
            pattern_supvsn = r'\"주관사\".*\<\/dt\>\n\<dd\>(.*)\n*\t*\<\/dd\>'
            pattern_addt_dtl = r'장소\<\/strong\>\n\<p\>\n[\n\t ]*(.*\s*.*\s*.*)\s*\<\/p\>'
            pattern_date = r'기간\<\/strong\>\n\<p\>(.*)\<\/p\>'
            pattern_time = r'시간\<\/strong\>\n\<p\>(.*)\<\/p\>'
            pattern_cost = r'\"입장료\".*\<\/dt\>\n\<dd\>(.*)\<\/dd\>'
            pattern_phone = r'전화번호\<\/strong\>\n\<p\>(.*)\<\/p\>'
            pattern_home = r'홈페이지\<\/strong\>\n\<p\>\<a.*\>(.*)\<\/a\>\<\/p\>'
            # pattern_ctn = r''
            ...
            if match:
                ...
                if len(addt_dtl) != 0:
                    str_addt_dtl = addt_dtl[0].replace('\n', '').replace('\t', '').replace('  ', '')
                else:
                    str_addt_dtl = ''
                ...
                if len(home[0]) > 0:
                    ...
                    print(encoding_url)
                    str_home_url = encoding_url.scheme + '://' + quote(encoding_url.netloc)
                    try:
